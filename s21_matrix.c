...
int iteraciyaDlyaLohov(matrix_t *A, matrix_t *B, int rows, int column) {
    if (A->rows == rows + 1 && A->columns == column + 1)
        return SUCCESS;
    ...
    return iteraciyaDlyaLohov(
        A,
        B,
        isEdgeColumn ? rows + 1 : rows,
...
int isCorrect(matrix_t *A, matrix_t *B) {
    if (!A || !B || !A->matrix || !B->matrix || A->rows < 1 || A->columns < 1 || B->rows < 1 || B->columns < 1)
        return FAILURE;
    return SUCCESS;
}
...
int resultHOF(matrix_t *A, matrix_t *B, matrix_t *result, double(*callback)(double, double), double* defaul) {
    if (!isCorrect(A, B))
        return INCORRECT_MATRIX;
    if (!isEqualSize(A, B))
        return CALCULATION_ERROR;
    ...
    for (int i = 0; i < A->rows; i++)
        for (int j = 0; j < A->columns; j++)
            result->matrix[i][j] = callback(A->matrix[i][j], defaul ? *defaul : B->matrix[i][j]);
